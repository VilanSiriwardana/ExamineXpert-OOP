...
@WebServlet("/StudentAnswersInsertServlet")
public class StudentAnswersInsertServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 // Get the student's answers for each question and insert them into the database.
	        ...
	        if (qids != null && sanswers != null && qids.length == sanswers.length) {
	            for (int i = 0; i < qids.length; i++) {
					...
					isTrue = PaperDBUtil.insertStudentAnswer(pid, mid, qid, sid, sanswer);
